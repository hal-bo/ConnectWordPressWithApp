...
require_once('load_csv/upload.php');
...
add_action('wp_ajax_tweet_voice', 'tweet_voice');  // ログイン状態のユーザーからのアクセスで動作する
add_action('wp_ajax_nopriv_tweet_voice', 'tweet_voice'); // 非ログインのユーザーからのアクセスで動作する
function tell_me() {
	$param = array(
	'posts_per_page' => '20',
	'tag' => ['spot'],
...
function tweet_voice() {
	$post_value = array(
		'post_title' => $_POST['voice'],
	...
	if( $insert_id != 0 ){
		update_post_meta($insert_id, 'name',$_POST['name']);
		update_post_meta($insert_id, 'lng', $_POST['lng']);
		update_post_meta($insert_id, 'lat',$_POST['lat']);
		update_post_meta($insert_id, 'voice',$_POST['voice']);
	...
	while ( $the_query->have_posts() ) : $the_query->the_post();
		$id = get_the_ID();
		$res[$j*4] = get_post_meta($id,'name',true);
		$res[$j*4+1] = get_post_meta($id,'lng',true);
		$res[$j*4+2] = get_post_meta($id,'lat',true);
		$res[$j*4+3] = get_post_meta($id,'voice',true);
		$j++;
	endwhile;
